...
- Polished cancelling the swipe-down chooser. Swiping back up now slides the minimize/close options up and fades them out (mirroring the way they drop in), instead of vanishing instantly. If you had aimed a snap zone before dipping into the chooser (a brief pause on the preview), cancelling returns the HUD to that zone so you can keep swiping elsewhere from there; a quick straight-down summon simply fades back to neutral. Cancelling never accidentally maximizes the window.
- The "Swipe-down deliberateness" slider now works in every mode and actually gates the action. It is no longer greyed out when Swipe down is set to Minimize, and the threshold is now applied when the swipe is released, not just to the live preview, so raising it really does require a longer, more deliberate downward pull before the window minimizes or closes (and lowering it makes a quick flick enough). Previously the slider had little effect in Minimize mode.
- Fixed in-app updates not relaunching Swoosh after a silent install. The installer's launch step was skipped during the silent update, so an updated Swoosh stayed closed until you started it again. Silent updates now relaunch the app automatically once the new version is in place. (This takes effect for updates installed with this version or later.)
- New "Swipe down" action. In Settings under Snapping (Snap behavior), you can now set what a downward swipe does: minimize the window (the default), close it, or "Let me choose". With "Let me choose", swiping down drops a small picker out below the window with two options, minimize on the left and a red close button on the right; lean toward one to pick it, or swipe back up to cancel and keep gesturing. A new "Swipe-down deliberateness" slider sets how far down you must pull before it engages, so an incidental downward motion during other gestures won't trigger it. The Gestures grid tile and the first-run tutorial both show the new minimize/close picker.
